...
## 12. Recipe：Telegram 行動維運與定時排程檢查

> 背景與現況：我已在 VM 部署 `agy-telegram-bot` 服務，並已於 `.env` 中設定 `ALLOWED_USER_ID` 與 `AGY_PERMISSION_MODE=safe`。
> 目標：規劃伺服器例行巡檢排程與 Telegram Bot 互動指令。
...
> 驗收：使用 `/status` 正確取得主機狀態；使用 `/schedule_add` 透過兩階段確認建立排程；於排程到期時收到格式正確的推播通知。
