# 第 7 章：Antigravity CLI (AGY) × Telegram 行動維運與自動排程

在前面的章節中，我們學會了如何建立「對外提供服務」的 Web AI 應用（Next.js + LiteLLM + Cloudflare Tunnel）。本章將帶領學員切入另一個極具企業價值的面向——**「對內 AIOps 行動伺服器維運」**。

我們將在晶創雲 Ubuntu VM 上部署一個輕量安全的 **Telegram → Antigravity CLI (`agy`) 橋接器**，讓管理員能夠在手機端透過 Telegram 隨身操控伺服器上的 AGY AI Agent，進行即時系統巡檢、故障排除，並利用**主機層級定時排程（Host-level Scheduler）**實現 24 小時無人值守的自動化維運！

---

## 1. 專案定位與分層架構哲學

### 1.1 Unix 哲學：輕量橋接，而非重複發明輪子
本專案**不是**另一個龐大的 AI Agent 框架（如 LangChain 或 AutoGen），而是遵循簡潔的 Unix 分工原則：
* **Telegram**：提供手機端隨身通訊介面與安全通道（長輪詢 Long Polling，**主機完全不需要開放對外 Inbound Port**）。
* **Python Bridge**：負責身分驗證（單一數字 User ID 白名單）、斜線指令路由、SQLite 排程器、執行鎖、超時控制與敏感字串脫敏。
...
* **概率性推理面（Probabilistic Reasoning Plane）**：任務的具體目標描述、伺服器巡檢邏輯、故障排查判斷，交由大語言模型（AGY）以**自然語言**進行彈性推理。

---

## 2. 🤖 自然語言 Prompt 配方（可直接複製給 AI Agent）

在 Antigravity 終端機中，你可以直接複製以下 Prompt 請 AI 協助完成部署與組態檢查：

### 模式 A：Telegram Bot 環境自動部署與組態檢查

```markdown
請協助我在這台晶創雲 Ubuntu VM 上部署 agy-telegram-bot 專案：
...
```markdown
請檢查目前 agy-telegram-bot 的安全設定模式：
...
### 模式 C：定時巡檢排程與靜默回報任務規劃

```markdown
請為我的伺服器規劃三個實用的 Telegram 定時維運排程：
...
1. **取得 Bot Token**：
   ...
   - 傳送 `/newbot`，依提示設定機器人名稱（如 `MyServer_Bot`）與 username。
...
2. Bot 啟動獨立進程要求 AGY 重寫為可重複執行的標準 Prompt 模板（此階段強制以 Safe 權限執行）。
3. Bot 在 Telegram 呈現 Inline Keyboard 按鈕：
   `[ ✅ 確認建立 ]` `[ ❌ 取消 ]`
...
> - **管理排程（查、刪、停、啟）** ➜ 敲 Telegram 專屬斜線指令（如 `/schedule_list`、`/schedule_delete 1`）。
...
| `/schedule_add` | 排程 | 建立定時任務（先經 AGY 整理提示詞並在 Telegram 預覽確認） | `/schedule_add */15 * * * * 檢查伺服器記憶體` |
...
* **正則脫敏**：內建過濾機制，自動將輸出中的 Telegram Bot Token、Bearer Token、API Key、密碼遮罩為 `[REDACTED]`。
* **檔案權限**：`.env` 與 `schedules.db` 強制限制為 `600`（僅服務使用者可讀寫）。

### 6.4 Sudo 權限安全原則
* **原則**：Bot 正常運作（含排程、Docker ps、磁碟查詢）**不需要** root 或 sudo 權限。
* **臨時維運需求**：若要讓 AGY 能自動執行 `sudo apt update` 或服務重啟，可手動設定免密碼 sudo，並於**任務完成後立即還原**：
  ```bash
...
### 🧪 Lab 1：基礎狀態查詢與延續對話測試
1. 在 Telegram 對 Bot 傳送 `/start` ➜ 確認顯示歡迎訊息與模式。
2. 傳送 `/status` ➜ 確認回報主機 Uptime、負載、磁碟、記憶體與 Docker 狀態。
3. 傳送 `請記住暗號 ALPHA` ➜ 確認 AGY 回應記住。
4. 傳送 `剛才暗號是什麼？` ➜ 確認 AGY 能延續對話上下文回覆 ALPHA。
5. 傳送 `/clear` ➜ 重置對話階段。
...
4. 到期時 ➜ Bot 自動主動推播巡檢報告至 Telegram。
...
| 現象 / 錯誤 | 原因分析 | 處置方式 |
|---|---|---|
| 輸入「停止排程」後 Bot 回覆已停止但排程仍在跑 | 純文字輸入被當成一般 AI 對話處理，AI 產生幻覺確認但無法修改底層 DB | 必須使用斜線指令 `/schedule_delete <ID>` 或 `/schedule_pause <ID>` |
...
- [ ] **服務常駐**：`agy-telegram.service` 成功啟動並設為開機自啟動（`systemctl is-active` 為 active）。
- [ ] **指令互動**：成功在 Telegram 執行 `/status` 並取得主機狀態回報。
- [ ] **上下文管理**：已驗證多輪對話與 `/clear` 重置上下文功能。
- [ ] **排程兩階段確認**：成功使用 `/schedule_add` 建立定時排程，並透過 Inline Keyboard 點擊確認。
- [ ] **靜默機制**：已測試 `[NO_REPORT]` 靜默回報機制，確認例行無異常時不洗版。
- [ ] **熔斷與恢復**：理解連續失敗 3 次自動熔斷保護與 `/schedule_resume` 恢復機制。

> [!TIP]
> 恭喜您完成全套課程！至此您不僅掌握了雲端基礎建設、多模型 API 治理、Web 全端 AI 應用開發與正式安全部署，更進一步解鎖了使用 AI Agent 進行行動端伺服器自主維運的進階架構能力！
